  ...
  const visuals = {
    ...
    Gauges: (
      <svg viewBox="0 0 160 100" style={{width:"100%",height:"100%"}}>
        ...
        <circle cx="80" cy="48" r="30" fill="none" stroke={nv} strokeWidth="0.4" opacity="0.1"/>
        ...
        {[0,6,12,18].map((i,j)=>{
          const a=(i/24)*Math.PI*2-Math.PI/2;
          return<text key={i} x={80+Math.cos(a)*19} y={48+Math.sin(a)*19+3} fontSize="6" fill={nv} textAnchor="middle" opacity="0.6">{j*25}</text>;
        })}
    ...
    "Surface Plates": (
      <svg viewBox="0 0 160 100" style={{width:"100%",height:"100%"}}>
        ...
        <rect x="22" y="84" width="116" height="3" rx="1" fill={nv} opacity="0.12"/>
        <circle cx="80" cy="45" r="4" fill="none" stroke={rd} strokeWidth="1.5" opacity="0.7"/>
        <line x1="80" y1="31" x2="80" y2="41" stroke={rd} strokeWidth="1" opacity="0.5" strokeDasharray="2,2"/>
        <line x1="65" y1="45" x2="76" y2="45" stroke={rd} strokeWidth="0.8" opacity="0.35" strokeDasharray="2,2"/>
        <line x1="84" y1="45" x2="95" y2="45" stroke={rd} strokeWidth="0.8" opacity="0.35" strokeDasharray="2,2"/>
      </svg>
    ),
    "Torque Tools": (
      <svg viewBox="0 0 160 100" style={{width:"100%",height:"100%"}}>
        <rect x="6" y="40" width="82" height="18" rx="9" fill={nv} opacity="0.18"/>
        <rect x="6" y="42" width="82" height="14" rx="7" fill={nv} opacity="0.12"/>
        <rect x="6" y="44" width="82" height="10" rx="5" fill={nv} opacity="0.1"/>
...
/* ═══════════════════
   FORM FIELD HELPER
   ═══════════════════ */
const Field = ({label, type="text", placeholder, defaultValue, rows}) => (
...
/* ═══════════════════
   SUCCESS STATE
   ═══════════════════ */
const SuccessMsg = ({title, msg, onClose}) => (
...
export default function App() {
  ...
  const [enquiryProd, setEnquiryProd] = useState(null);
  ...
  const updateQty  = (id,q) => q<1 ? removeItem(id) : setCart(c=>c.map(i=>i.id===id?{...i,qty:q}:i));
  const cartCount  = cart.reduce((s,i)=>s+i.qty,0);
  const cartTotal  = cart.reduce((s,i)=>s+i.price*i.qty,0);
  const filtered   = activeCat==="All" ? PRODUCTS : PRODUCTS.filter(p=>p.cat===activeCat);

  const openEnquiry = (prod=null) => { setEnquiryProd(prod); setEnquiryDone(false); setModal("enquiry"); };
    ...
    <>
      ...
      <div style={{minHeight:"100vh",background:"var(--bg)"}}>
          ...
          <div style={{maxWidth:1280,margin:"0 auto",padding:"0 2rem",display:"flex",alignItems:"center",gap:"1.5rem",height:68}}>
            ...
            <nav style={{display:"flex",gap:"0.1rem",marginLeft:"auto",alignItems:"center"}}>
              {["Products","About"].map(l=>(
                <a key={l} href={`#${l.toLowerCase()}`} style={{padding:"0.4rem 0.9rem",color:"var(--mu)",fontSize:"0.85rem",transition:"color .2s",cursor:"pointer"}}
                  onMouseEnter={e=>e.target.style.color="var(--nv)"} onMouseLeave={e=>e.target.style.color="var(--mu)"}>{l}</a>
              ))}
              <button onClick={()=>openEnquiry()} style={{padding:"0.4rem 0.9rem",color:"var(--mu)",fontSize:"0.85rem",background:"none",border:"none",cursor:"pointer",transition:"color .2s"}}
                onMouseEnter={e=>e.target.style.color="var(--nv)"} onMouseLeave={e=>e.target.style.color="var(--mu)"}>Enquire</button>
            </nav>
            <button onClick={()=>setCartOpen(true)} aria-label="Open cart"
              style={{position:"relative",padding:"0.55rem 0.85rem",background:"var(--nv)",border:"none",cursor:"pointer",color:"white",borderRadius:3,display:"flex",alignItems:"center",gap:"0.5rem",fontSize:"0.78rem",fontWeight:500,transition:"background .2s",flexShrink:0}}
              onMouseEnter={e=>e.currentTarget.style.background="var(--nvl)"} onMouseLeave={e=>e.currentTarget.style.background="var(--nv)"}>
        ...
        <section style={{background:"var(--nv)",color:"white",padding:"5.5rem 2rem 6rem",position:"relative",overflow:"hidden"}}>
          ...
          {/* decorative ring */}
          <div style={{position:"absolute",right:"7%",top:"50%",transform:"translateY(-50%)",width:280,height:280,border:"1px solid rgba(255,255,255,.06)",borderRadius:"50%",pointerEvents:"none"}}/>
          <div style={{position:"absolute",right:"7%",top:"50%",transform:"translate(-10%,-50%)",width:220,height:220,border:"1px solid rgba(255,255,255,.04)",borderRadius:"50%",pointerEvents:"none"}}/>
          {/* corner accent */}
          ...
          <div style={{position:"relative",zIndex:1,maxWidth:1280,margin:"0 auto"}}>
            ...
            <div style={{display:"flex",gap:"1rem",flexWrap:"wrap"}}>
              <a href="#products" style={{padding:"0.75rem 2.2rem",background:"var(--rd)",color:"white",fontSize:"0.85rem",fontWeight:500,letterSpacing:"0.04em",transition:"background .2s",cursor:"pointer",display:"inline-block"}}
                onMouseEnter={e=>e.target.style.background="var(--rdl)"} onMouseLeave={e=>e.target.style.background="var(--rd)"}>Browse Catalogue</a>
              <button onClick={()=>openEnquiry()} style={{padding:"0.75rem 2rem",background:"transparent",color:"white",border:"1px solid rgba(255,255,255,.25)",cursor:"pointer",fontSize:"0.85rem",transition:"all .2s"}}
                onMouseEnter={e=>{e.target.style.borderColor="rgba(255,255,255,.7)";e.target.style.background="rgba(255,255,255,.06)"}} onMouseLeave={e=>{e.target.style.borderColor="rgba(255,255,255,.25)";e.target.style.background="transparent"}}>Request Quotation →</button>
            </div>
          </div>
        </section>

        {/* ──── TRUST STRIP ─────────────────────────────── */}
        <div style={{background:"#0A1A33",padding:"1.1rem 2rem",display:"flex",flexWrap:"wrap",justifyContent:"center",gap:0}}>
            ...
            <div style={{display:"flex",gap:"0.4rem",flexWrap:"wrap"}}>
              {CATS.map(c=>(
                <button key={c} onClick={()=>setActiveCat(c)}
                  style={{padding:"0.38rem 1.1rem",background:activeCat===c?"var(--nv)":"var(--sf)",border:`1px solid ${activeCat===c?"var(--nv)":"var(--br)"}`,cursor:"pointer",fontSize:"0.78rem",color:activeCat===c?"white":"var(--mu)",transition:"all .2s"}}>
                  {c}
                </button>
              ))}
            </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))",gap:"1.25rem"}}>
            {filtered.map(p=>(
              <article key={p.id} onClick={()=>setSelectedProd(p)}
                ...
                onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-4px)";e.currentTarget.style.boxShadow="0 14px 36px rgba(13,32,64,.1)";e.currentTarget.style.borderColor="#C8C4BB"}}
                onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="";e.currentTarget.style.borderColor="var(--br)"}}>
                {/* visual area */}
                <div style={{position:"relative",background:"#EEECEA",height:155,padding:"1.5rem 2rem",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
                  ...
                  <span style={{position:"absolute",top:"0.65rem",left:"0.65rem",background:"white",border:"1px solid var(--br)",fontSize:"0.6rem",padding:"0.18rem 0.5rem",color:"var(--mu)",fontWeight:500,letterSpacing:"0.02em"}}>{p.brand}</span>
                ...
                <div style={{padding:"1.25rem"}}>
                  ...
                  <div style={{display:"flex",gap:"0.5rem"}} onClick={e=>e.stopPropagation()}>
                    <button onClick={()=>openEnquiry(p)}
                      style={{flex:1,padding:"0.5rem",background:"white",border:"1px solid var(--nv)",color:"var(--nv)",cursor:"pointer",fontSize:"0.74rem",fontWeight:500,transition:"all .2s"}}
                      onMouseEnter={e=>{e.target.style.background="var(--nv)";e.target.style.color="white"}} onMouseLeave={e=>{e.target.style.background="white";e.target.style.color="var(--nv)"}}>Enquire</button>
                    <button disabled={!p.stock} onClick={()=>addToCart(p)}
                      style={{flex:1,padding:"0.5rem",background:p.stock?"var(--nv)":"#CBD5E1",border:`1px solid ${p.stock?"var(--nv)":"#CBD5E1"}`,color:"white",cursor:p.stock?"pointer":"not-allowed",fontSize:"0.74rem",fontWeight:500,transition:"background .2s"}}
                      onMouseEnter={e=>{if(p.stock)e.target.style.background="var(--nvl)"}} onMouseLeave={e=>{if(p.stock)e.target.style.background="var(--nv)"}}>
          ...
          <div style={{maxWidth:1280,margin:"0 auto"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4rem",alignItems:"center"}}>
              <div>
                ...
                <p style={{fontSize:"0.875rem",lineHeight:1.8,color:"var(--mu)",marginBottom:"1.5rem"}}>All instruments are supplied with original manufacturer calibration certificates traceable to national standards. Technical support and on-site calibration services available.</p>
                <button onClick={()=>openEnquiry()} style={{padding:"0.72rem 1.75rem",background:"var(--nv)",color:"white",border:"none",cursor:"pointer",fontSize:"0.85rem",fontWeight:500,transition:"background .2s"}}
                  onMouseEnter={e=>e.target.style.background="var(--nvl)"} onMouseLeave={e=>e.target.style.background="var(--nv)"}>Get in Touch</button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1px",background:"var(--br)"}}>
                {[["Mitutoyo","Micrometers, Calipers, Height Gauges"],["Tohnichi","Torque Wrenches, Screwdrivers"],["Niigata Seiki","Calipers, Gauges, Rulers"],["Shinto","Surface Plates, Granite Tools"]].map(([b,d])=>(
        ...
        <footer style={{background:"#08162A",color:"rgba(255,255,255,.45)",padding:"3.5rem 2rem 0"}}>
          <div style={{maxWidth:1280,margin:"0 auto",display:"grid",gridTemplateColumns:"2fr 1.2fr 1fr 1fr",gap:"2.5rem",paddingBottom:"2.5rem"}}>
            <div>
              ...
              <p style={{fontSize:"0.8rem",lineHeight:1.75,color:"rgba(255,255,255,.38)",maxWidth:260}}>Specialising in Japanese precision metrology and torque instruments for Singapore's precision engineering sector.</p>
            </div>
            <div>
            ...
            <div>
              ...
              {["Mitutoyo","Tohnichi","Niigata Seiki","Shinto","Insize"].map(b=><p key={b} style={{fontSize:"0.79rem",marginBottom:"0.3rem"}}>{b}</p>)}
            </div>
            <div>
              ...
              {["Product Catalogue","Request Quotation","Calibration Services","Technical Support","After-Sales Service"].map(l=>(
                <p key={l} style={{fontSize:"0.79rem",marginBottom:"0.3rem",cursor:"pointer",color:"rgba(255,255,255,.4)"}}>{l}</p>
              ))}
            </div>
          </div>
        ...
        {/* ──── CART DRAWER ──── */}
        {cartOpen && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.52)",zIndex:200,display:"flex",justifyContent:"flex-end",backdropFilter:"blur(2px)"}} onClick={()=>setCartOpen(false)}>
            <div style={{width:400,maxWidth:"100vw",background:"var(--sf)",height:"100%",display:"flex",flexDirection:"column",animation:"slideIn .25s ease"}} onClick={e=>e.stopPropagation()}>
              <style>{`@keyframes slideIn{from{transform:translateX(100%)}to{transform:translateX(0)}}@keyframes fadeUp{from{transform:translateY(16px);opacity:0}to{transform:translateY(0);opacity:1}}`}</style>
              ...
              {cart.length>0 && (
                <div style={{padding:"1.25rem 1.5rem",borderTop:"1px solid var(--br)"}}>
                  ...
                  <button onClick={()=>{setCartOpen(false);setCheckoutDone(false);setModal("checkout")}}
                    style={{width:"100%",padding:"0.75rem",background:"var(--nv)",color:"white",border:"none",cursor:"pointer",fontSize:"0.85rem",fontWeight:500,transition:"background .2s"}}
                    onMouseEnter={e=>e.target.style.background="var(--nvl)"} onMouseLeave={e=>e.target.style.background="var(--nv)"}>Proceed to Checkout</button>
                </div>
              )}
            </div>
          </div>
        ...
        {selectedProd && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.52)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem",backdropFilter:"blur(2px)"}} onClick={()=>setSelectedProd(null)}>
            <div style={{background:"var(--sf)",padding:"2rem",maxWidth:700,width:"100%",maxHeight:"90vh",overflowY:"auto",position:"relative",animation:"fadeUp .22s ease",borderTop:"3px solid var(--nv)",display:"grid",gridTemplateColumns:"220px 1fr",gap:"1.75rem",alignItems:"start"}} onClick={e=>e.stopPropagation()}>
              <button onClick={()=>setSelectedProd(null)} style={{position:"absolute",top:"1rem",right:"1rem",background:"none",border:"none",cursor:"pointer",color:"var(--mu)",display:"flex",padding:"0.2rem"}}><Ic.X/></button>
              ...
              <div>
                ...
                <div style={{display:"flex",gap:"0.5rem"}}>
                  <button onClick={()=>{setSelectedProd(null);openEnquiry(selectedProd)}}
                    style={{flex:1,padding:"0.55rem",background:"white",border:"1px solid var(--nv)",color:"var(--nv)",cursor:"pointer",fontSize:"0.78rem",fontWeight:500,transition:"all .2s"}}
                    onMouseEnter={e=>{e.target.style.background="var(--nv)";e.target.style.color="white"}} onMouseLeave={e=>{e.target.style.background="white";e.target.style.color="var(--nv)"}}>Enquire</button>
        ...
        {/* ──── CHECKOUT MODAL ──── */}
        {modal==="checkout" && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.52)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem",backdropFilter:"blur(2px)"}} onClick={closeModal}>
                ...
                : <>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"2.5rem",marginBottom:"1rem"}}>
                    <div>
                      ...
                      <Field label="Notes / Special Requirements" placeholder="Calibration certs, delivery date, training..." rows={2}/>
                    </div>
                    <div>
                      ...
                      <div style={{background:"#F0EFE9",border:"1px solid var(--br)",padding:"0.85rem",marginTop:"0.75rem"}}>
                        <p style={{fontSize:"0.75rem",color:"#5A5040",lineHeight:1.65}}>No payment is collected online. A formal quotation will be issued by our team. Prices are indicative and subject to stock availability.</p>
                      </div>
                    </div>
                  </div>
                  <button onClick={()=>setCheckoutDone(true)}
                    style={{padding:"0.75rem 2.5rem",background:"var(--nv)",color:"white",border:"none",cursor:"pointer",fontSize:"0.85rem",fontWeight:500,marginTop:"0.5rem",transition:"background .2s"}}
                    onMouseEnter={e=>e.target.style.background="var(--nvl)"} onMouseLeave={e=>e.target.style.background="var(--nv)"}>Submit Order Request</button>
                </>
              }
            </div>
          </div>
              ...
              {enquiryDone
                ...
                : <>
                  ...
                  <button onClick={()=>setEnquiryDone(true)}
                    style={{width:"100%",padding:"0.75rem",background:"var(--nv)",color:"white",border:"none",cursor:"pointer",fontSize:"0.85rem",fontWeight:500,marginTop:"0.25rem",transition:"background .2s"}}
                    onMouseEnter={e=>e.target.style.background="var(--nvl)"} onMouseLeave={e=>e.target.style.background="var(--nv)"}>Send Enquiry</button>
                </>
              }
            </div>
          </div>
